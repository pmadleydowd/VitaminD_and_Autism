cap log using "$Logdir\LOG_an_sensitivity_spline.txt", text replace

********************************************************************************
* Author: 		Paul Madley-Dowd
* Date: 		02 August 2021
* Description: 	Sensitivity for spline analyses 
********************************************************************************
* Contents
* 1 Set up environment and read in data
* 2 Perform spline analyses for logistic and then regression models
...
********************************************************************************
* 2 Perform spline analyses for logistic and then linear regression models
********************************************************************************
set scheme s2mono
summarize sa_20wk_VitDtot_10, det

* Create splines
gen rnd_sadj = round(sa_20wk_VitDtot_10,0.01)	

forvalues n=3(1)4 {
	mkspline sadj_spline = rnd_sadj, nknots(`n') cubic displayknots
	...
	*/ title("`tit'", size(small)) /*		
		...
		*/ name(fig`i', replace)
		...
		graph export "$Graphdir\sens`n'_spline_sadj_`outcome'.tif", height(2000) width(1600) replace
			
		drop vitd est1 lci1 uci1 yline
	}
		
	** Perform spline analyses for linear regression models
	* Create model 	
	...
	*/ name(fig6, replace)				
	...
	graph display, ysize(20) xsize(16)
	graph export "$Graphdir\sens`n'_spline_sadj_fm.tif", height(2000) width(1600) replace
		
	drop vitd est1 lci1 uci1 yline
	drop sadj_spline*
	
	graph combine fig1 fig2 fig3 fig4 fig5 fig6 , cols(3)
	graph export "$Graphdir\sens`n'_spline_combined.tif", height(2000) width(3000) replace
